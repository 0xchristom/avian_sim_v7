...
pub fn sample_age(rng: &mut SimRng) -> Age {
    ...
    Age { years, months: (total_months % 12) as u8, vitality: calibration::vitality_at(years) }
}
...
pub fn mass_from_age(age: &Age, rng: &mut SimRng) -> Mass {
    ...
    let base_mass = if age.years < 1.0 {
        calibration::FLEDGLING_MASS_G
            + (calibration::ADULT_MASS_G - calibration::FLEDGLING_MASS_G) * (age.years / 1.0).clamp(0.0, 1.0)
...
pub fn spawn_agent(
    ...
    let e = world.spawn((
        Position(pos), Velocity(nalgebra::Vector2::zeros()), Heading(rng.gen_range(0.0..std::f64::consts::TAU)),
        mass, age,
        Metabolism {
            bmr_watts: calibration::bmr_for_mass(mass.current_g),
            energy_kj: 40.0 + rng.gen_range(0.0..20.0), hunger: 0.2,
            crop_count: crop_max / 2, gizzard_count: 3, crop_max, last_peck_time: 0.0,
        },
        FSMState::Spacer,
        LevyState { remaining_dist: 0.0, target_heading: rng.gen_range(0.0..std::f64::consts::TAU) },
        Mobility { max_speed_ms: calibration::WALK_SPEED_MS, max_angular_speed_rads: 2.0, acceleration_ms2: 10.0 * mass_kg.powf(-0.25) },
        Vision { fov_degrees: calibration::VISION_FOV_DEGREES, fovea_resolution: 1.0, blind_front_degrees: 20.0, blind_rear_degrees: 30.0 },
        HeadBob { phase: HeadBobPhase::Hold, offset: nalgebra::Vector2::zeros(), time_in_phase: 0.0, hold_duration: 0.1, thrust_duration: 0.05 },
