...
fn multiple_predators_all_move() {
    ...
    let u1 = sim
        .world
        .get::<&avian_core::components::AgentUid>(e1)
        .unwrap()
        .0
        .clone();
    let u2 = sim
        .world
        .get::<&avian_core::components::AgentUid>(e2)
        .unwrap()
        .0
        .clone();
    ...
    for _ in 0..600 {
        ...
        if let Some(p) = snap.predators.iter().find(|p| p.uid == u2) {
            if (p.pos[0] - 25.0).abs() + (p.pos[1] - 16.0).abs() > 0.5 {
                moved2 = true;
            }
        }
    }
...
fn sequentially_deployed_predators_move() {
    ...
    let u3 = sim
        .world
        .get::<&avian_core::components::AgentUid>(e3)
        .unwrap()
        .0
        .clone();
    let p2 = sim
        .world
        .get::<&avian_core::components::Position>(e2)
        .unwrap()
        .0;
    let p3 = sim
        .world
        .get::<&avian_core::components::Position>(e3)
        .unwrap()
        .0;
    ...
    for _ in 0..600 {
        ...
        if let Some(p) = snap.predators.iter().find(|p| p.uid == u3) {
            if (p.pos[0] - p3.x).abs() + (p.pos[1] - p3.y).abs() > 0.5 {
                moved3 = true;
            }
        }
    }
