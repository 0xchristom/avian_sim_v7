...
use components::{Position, Heading, Metabolism, Mass, Age, Velocity, FSMState};
...
impl Simulation {
    ...
    pub fn snapshot(&self) -> SimulationSnapshot {
        ...
        for (id, (pos, head, vel, meta, mass, age, fsm)) in self.world.query::<(&Position, &Heading, &Velocity, &Metabolism, &Mass, &Age, &FSMState)>().iter() {
            agents.push(AgentSnapshot {
                ...
                hunger: meta.hunger,
                fsm_state: fsm.0.clone(),
            });
        }
        SimulationSnapshot { frame: self.time.frame, time_us: self.time.time_us, agents }
    }
}